...
function dateToTimestamp(date) {
  return Date.parse(date) - new Date('1970-01-01T00:00:00Z').getTime();
}
...
function getNextFriday(date) {
  ...
  const nextFriday = new Date(
    dateTime.setDate(
      dateTime.getDate() + ((7 - dateTime.getDay() + 5) % 7 || 7)
    )
  );
...
function formatDate(date) {
  ...
  const format = new Date(date).toLocaleString('en-US', options);
...
function getWeekNumberByDate(date) {
  ...
  const differentYear = date - startYear;

  const dayOnMs = 1000 * 60 * 60 * 24;
  const week = Math.ceil(
    (differentYear / dayOnMs + startYear.getUTCDay() + 1) / 7
  );
